function Cell=DoG_withS(Em)

[cnAll,snAll,cpAll,spAll]=RandomizeConeInputs_Gauss1(Em);
pause;
%%%%%%%%%% MODEL UNIVERSE %%%%%%%%%%%%%%
...
CSRadRatio=10; %Let's also say that a surround radius is ~7x greater than a center radius (Croner & Kaplan, Vis. Res. 1995)
...
p=pdf('bino',0:ConesToCenter,ConesToCenter,0.5833)'*pdf('bino',0:ConesToSurround,ConesToSurround,0.5833); %Probability surface of L:M weights in C and S
...
% Plot the RF (total and components)
% figure;
% hold on; 
...
subplot(2,3,1);
Ymax=max([TotalLData.Amplitude TotalMData.Amplitude TotalSData.Amplitude]);
if Ymax==0
    Ymax=10;
end
...
PlotSPhase=semilogx(Freqs,abs(TotalSData.Phase),'bo');
title({centerstrength;surroundstrength});
xlabel('Stimulus frequency (cpd)');
ylabel('Phase (degrees)'); 
axis([0.005 150 -20 200]);
set(gca,'TickDir','in','TickLength', [.005 .005]);box off
legend([PlotLPhase,PlotMPhase PlotSPhase],'Location','northeast')
...
% %Write cell parameters and response functions to file
% AmpIndex=TotalData.Amplitude(1)-LMDiffData.Amplitude(1);
% PhaseIndex=TotalLData.Phase(1)-TotalMData.Phase(1);
% 
...
Cell.ResponseFunctions.SIsoResponse.Amplitude=TotalSData.Amplitude;
Cell.ResponseFunctions.SIsoResponse.Phase=TotalSData.Phase;
