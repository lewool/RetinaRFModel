...
function Cell=GenerateCell_EllipticalCenter(Em)
%% GENERATE THE PATCH
...
%Build a ~2:1 ellipse to capture center cones
CenterEllipseRadiusA=0.003835*Em.^1.327; %in mm
CenterEllipseRadiusB=0.001952*Em.^1.328; %in mm
[EllipseX,EllipseY]=ellipse(CenterEllipseRadiusA,CenterEllipseRadiusB,0,0,0);
...
EllipseIndex=find(CenterX(:,1));
% CenterX(~any(CenterX,2),:)=NaN;
CenterX=CenterX(EllipseIndex,:);
% ConesToCenter=length(CenterX(EllipseIndex,:));
...
% CenterEllipticalKernel(isnan(CenterEllipticalKernel))=0;
...
fullfieldCenter(EllipseIndex,:)=fullfieldCenter(EllipseIndex,:)+NormalizedCenterEllipticalKernel;
