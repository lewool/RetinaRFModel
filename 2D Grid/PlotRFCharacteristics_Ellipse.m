function PlotRFCharacteristics_Ellipse(Cell)
...
CEllipseIndex=ImportCell.Center.EllipseIndex;
SEllipseIndex=ImportCell.Surround.EllipseIndex;
...
NormalizedCenterEllipticalKernel=ImportCell.Center.CenterKernel;
NormalizedSurroundEllipticalKernel=ImportCell.Surround.SurroundKernel;
...
subplot(1,2,1);
scatter(SortedTotalMosaic(CEllipseIndex,1),SortedTotalMosaic(CEllipseIndex,2),30,NormalizedCenterEllipticalKernel,'fill')
axis([-max(CenterX(:,3))*1.1 max(CenterX(:,3))*1.1 -max(CenterX(:,3))*1.1 max(CenterX(:,3))*1.1]);
axis square
xlabel('x (mm)');
ylabel('y (mm)'); 
title(strcat('Center (',num2str(ConesToCenter),' cones)'));
subplot(1,2,2);
scatter(SortedTotalMosaic(SEllipseIndex,1),SortedTotalMosaic(SEllipseIndex,2),30,NormalizedSurroundEllipticalKernel,'fill')
axis([-max(SurroundX(:,3))*1.1 max(SurroundX(:,3))*1.1 -max(SurroundX(:,3))*1.1 max(SurroundX(:,3))*1.1]);
axis square
xlabel('x (mm)');
ylabel('y (mm)'); 
...
axis([-max(SurroundX(:,3))*2 max(SurroundX(:,3))*2 -max(SurroundX(:,3))*2 max(SurroundX(:,3))*2]);
axis square
xlabel('x (mm)');
ylabel('y (mm)'); 
