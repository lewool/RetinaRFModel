...
function Cell=GenerateCell_CrysS(Em)
%% GENERATE THE PATCH
...
%Note that H1 cells do not connect to S cones...
SurroundSCoordWeights=zeros(length(SCoords),1);
spS=0;
%And adjust the L/M weights in the surround accordingly
corTotal=spL+spM;
spL=spL/(corTotal);
spM=spM/(corTotal);
