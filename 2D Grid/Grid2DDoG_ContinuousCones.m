function Cell=Grid2DDoG(Em,Universe,thetas,SpatFreqs,SFLines,OriCircles)
%% GENERATE CELL
...
SCoordsDeg=(SCoords*1000)/200;
SCoordsDeg(isnan(SCoordsDeg(:,1)),:) = [];
...
CenterRFX=RFX(find(abs(RFX)<CenterRadiusDeg));
CenterRFX=CenterRFX(1:end-1);
[CenterRFXm, CenterRFYm] = meshgrid(CenterRFX, CenterRFX);

SurroundRFX=RFX(find(abs(RFX)<SurroundRadiusDeg));
SurroundRFX=SurroundRFX(1:end-1);
[SurroundRFXm, SurroundRFYm] = meshgrid(SurroundRFX, SurroundRFX);

CenterKernel=exp(-(CenterRFXm.^2)/(2*CenterRadiusDeg^2)-(CenterRFYm.^2)/(2*CenterRadiusDeg^2));
CenterKernel=padarray(CenterKernel,[length(RFXm)/2-length(CenterKernel)/2 length(RFXm)/2-length(CenterKernel)/2]);
SurroundKernel=exp(-(SurroundRFXm.^2)/(2*SurroundRadiusDeg^2)-(SurroundRFYm.^2)/(2*SurroundRadiusDeg^2));
SurroundKernel=padarray(SurroundKernel,[length(RFXm)/2-length(SurroundKernel)/2 length(RFXm)/2-length(SurroundKernel)/2]);


%CENTER RESPONSE PROFILES

%Apply a Gaussian to each center L cone coordinate...
TruncatedLCoordsDeg=LCoordsDeg(1:500,:);
if size(TruncatedLCoordsDeg,1)>=1
    for t=1:size(TruncatedLCoordsDeg,1)
    CgaussL(:,:,t)=exp(-((RFXm-TruncatedLCoordsDeg(t,1)).^2/2/ConeSigma^2 + (RFYm-TruncatedLCoordsDeg(t,2)).^2/2/ConeSigma^2));
    end
    AllL=sum(CgaussL,3);
else
    AllL=zeros(length(RFXm));
end

%...and for each center M cone coordinate
TruncatedMCoordsDeg=MCoordsDeg(1:500,:);
if size(TruncatedMCoordsDeg,1)>=1
    for t=1:size(TruncatedMCoordsDeg,1)
    CgaussM(:,:,t)=exp(-((RFXm-TruncatedMCoordsDeg(t,1)).^2/2/ConeSigma^2 + (RFYm-TruncatedMCoordsDeg(t,2)).^2/2/ConeSigma^2));
    end
    AllM=sum(CgaussL,3);
else
    AllM=zeros(length(RFXm));
end
